...
const findProductByID = (id) =>
  products.filter((product) => product.productId === Number(id));
...
app.get("/products/:id", function (req, res) {
  ...
  // нема сенсу робити щось далі якщо не був переданий id
  // та в цілому цей запит не спрацює якщо його не буде, тому цю
  // перевірку можна прибрати
  ...
  readFile({
    ...
    callback: (error, data) => {
      if (error) {
        res
          .status(500)
          .json({ ...error, message: "Upsss...Something happend" });
        return;
      }
